        ...
        >
          <div className="aspect-square mb-1 relative">
            <img 
              src={track.image} 
              alt={track.name}
              className="w-full h-full object-cover shadow-lg rounded-xs"
            />
          </div>
          <div className="">
            ...
            <p className="text-xs text-white/75 truncate">
              {track?.date ? formatDate(track?.date) : 'Now playing'}
            </p>
          </div>
        </div>
      ))}
...
const BooksSection = ({ books }: { books: BookInfo[] }) => (
  ...
  >
    <div className='sticky border-b pb-2'>
      ...
      <p className='text-[#efdfba] text-opacity-50 font-extralight'>My favourite books</p>
    </div>
    ...
    <main className='min-h-screen relative'>
      ...
      <div className='container mx-auto px-4 py-16 w-full max-w-3xl space-y-4'>
        ...
        {recentTracks && recentTracks.length > 0 && (
          <div>
            <SongsSection tracks={recentTracks} />
          </div>
        )}
      </div>
    </main>
